...
h = 1;  %συντελεστής h του kernel

f = zeros(3000,1);   %αρχικοποίηση του διανύσματος f μήκους 3*1000 γιατί επιλέχθηκε βήμα 0.001 για το x
...
for i=-1:0.001:1.999    %βρόγχος για κάθε i από -1 εώς 2 με βήμα 0.01
    ...
    for j = 1:n                                            
        f(index) = f(index) + laplacian_kernel(i-x(j),h);    %υπολογισμός προσέγγισης f(x)
    end
...
plot(position,f, 'b', position, pdf(makedist('Uniform'),position), 'r')    %plot και σύγκριση με την uniform 

function val = laplacian_kernel(x,h)     %συνάρτηση υπολογισμού laplacian kernel
    val = (1/(2*h)) * exp(-(abs(x))/(h));
end
